...
export class ItemsListComponent implements OnInit {
  ...
  async getItems(page: number = 0, limit: number = 1000, searchTerm:string = '', typeFilter: string = ''): Promise<void> {
    if(searchTerm!==''&&typeFilter===''){
    ...
    if(searchTerm===''&&typeFilter!==''){
    return new Promise((resolve, reject) => {
    this.apiService.Read(`/EducationalResource/getAll?page=${page}&limit=${limit}&typeFilter=${typeFilter}`).
        .subscribe({
      next: (response) => {
        console.log('i this is the response: ', response);
