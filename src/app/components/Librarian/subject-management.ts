...
export class SubjectManagementComponent implements OnInit {
  ...
  confirmDeleteSubject(): void {
    if (this.confirmSubject) {
      ...
      this.apiService.Delete('/subjects/deleteSubject', { id: this.confirmSubject._id }).subscribe({
        next: () => {
          // עדכון הרשימה המקומית לאחר המחיקה
          this.subjects = this.subjects.filter(subject => subject._id !==subjectToDelete);
  ...
  saveSubject(subject: Subject): void {
    if (subject.name !== subject.originalName || subject.description !== subject.originalDescription) {
      // שינוי: שליחה דרך BODY במקום PARAM
      this.apiService.Put('/subjects/updateSubject', { id: subject._id, name: subject.name, description: subject.description }).subscribe({
        next: () => {
